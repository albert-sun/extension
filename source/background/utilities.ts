...
export async function broadcastMessage(handler: string, ...args: any) {
    const request: MessageRequest = {
        sender: "background",
        handler, args,
    };

    const response = browser.runtime.sendMessage(request);
...
export async function mirrorStorage<Type>(storageKey: string, defaultValue: Type, broadcast: boolean = true): Promise<Type> {
